...
def _parse_args():
    ...
    parser.add_argument(
        "--input_dir",
        type=str,
        default="stories_test/images",
        help="Directory containing input images.")

    parser.add_argument(
        "--output_dir",
        type=str,
        default="stories_test/videos",
        help="Directory to save the output videos.")

    parser.add_argument(
        "--json_file_path",
        type=str,
        default="stories_test/prompts.json",
...
def generate(args):
    ...
    if "t2v" in args.task or "t2i" in args.task:
        ...
        # Load the prompt list from JSON file and the anchoring images
        # Assuming the JSON file is structured as a list of prompts
        # input_dir = "stories_test/images"
        # output_dir = "stories_test/videos"
        # json_file_path = "stories_test/prompts.json"  
        ...
        for entry in story_list:
            video_list = []
            if rank == 0:
                prompt_list = entry["prompts"]
                img = [Image.open(os.path.join(args.input_dir, entry["id"])).convert("RGB")]
            else:
                prompt_list = [None]*len(entry["prompts"])
                img =[None]

            # Broadcast the prompt list to all ranks
            dist.broadcast_object_list(prompt_list, src=0)
            dist.broadcast_object_list(img, src=0)

            anchor_img = img[0]
       
            if rank == 0:
                if isinstance(anchor_img, Image.Image):
                    original_width, original_height = anchor_img.size
                else:
                    # If it's already a tensor, get dimensions
                    if len(anchor_img.shape) == 3:  # C,H,W format
                        original_height, original_width = anchor_img.shape[1:3]
                    else:
                        # Default fallback
            ...
            for segment in range(len(prompt_list)):
                if segment == 0:
                    input_frame = anchor_img
                print(f"Generating video segment {segment + 1}/{len(prompt_list)}")
                ...
                if rank == 0:
                    ...
                    print(f"Frame bucket {segment + 1}/{len(prompt_list)}")
                else:
                    frame_bucket = [None]

                # This will replace frame_bucket[0] on all ranks with rank-0’s frame
                print(f"Broadcasting frame bucket {segment + 1}/{len(prompt_list)}")
                dist.broadcast_object_list(frame_bucket, src=0)
                print(f"Frame bucket {segment + 1}/{len(prompt_list)} broadcasted")
                input_frame = frame_bucket[0]
            # Concatenate all video segments
            if rank == 0:
                video = torch.cat(video_list, dim=1)
                # save the video
                video_path = os.path.join(args.output_dir, f"{entry['id'].rsplit('.', 1)[0]}.mp4")
                cache_video(
                    tensor=video[None],
