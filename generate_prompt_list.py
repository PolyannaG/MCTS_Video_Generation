...
def generate(args):
    ...
    if "t2v" in args.task or "t2i" in args.task:
        ...
        input_dir = "prompts_test/images"
        output_dir = "prompts_test/videos"
        json_file_path = "prompts_test/prompts.json"  
        with open(json_file_path, "r") as f:
            prompts = json.load(f)
        os.makedirs(output_dir, exist_ok=True)

        for entry in prompts:
            image_path = os.path.join(input_dir, entry["image_path"])
            prompt = entry["text"]
            ...
            if rank == 0:   
                cache_video(
                tensor=video[None],
                save_file=f"{output_dir}/{entry['image_path'].rsplit('.', 1)[0]}.mp4",
                fps=cfg.sample_fps,
                nrow=1,
                normalize=True,
                value_range=(-1, 1))
